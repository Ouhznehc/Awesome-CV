...
%---------------------------------------------------------
  \cventry
    {National level Innovative Entrepreneurial Training Program} % Job title
    {Resource-based Android Application Repackaging Detection} % Organization
    {Nanjing, China} % Location
    {Sep. 2022 - Nov. 2023} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Developed and tested a repackaging detection program based on Android application resources using C language
        and Python; analyzed and extracted resource features using the dHash, LSH, K-means clustering algorithm and generated software birthmarks for comparison to improve the recall rate of repackaging application detection from 3\% to 97\%, precision rate from 20\% to 90.6\%.}
        \item {Promoted the program with Prof.Jun Ma to prepare for patent application and paper publication.}
      \end{cvitems}
    }
